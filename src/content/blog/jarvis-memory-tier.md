...
AI를 쓰다 보면 나눴던 대화를 못 기억하거나 잘못 기억하는 부분이 있는 것 같았다

JARVIS도 RAG로 메모리를 검색해서 Claude에게 넘기는 구조라, 똑같은 문제를 겪을 걸 알고 있었다

메모리가 쌓일수록 오래된 메모리와 최근 메모리가 유사도 점수로만 경쟁하다 보니, 핀해둔 핵심 정보(CORE)조차 검색 순위에서 밀려서 Claude에게 전달이 안 되는 경우가 생겼다
